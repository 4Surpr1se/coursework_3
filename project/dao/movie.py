...
class MovieDAO:
    ...
    def get_all(self, user_offset=0, user_limit=None):
        return self.session.query(Movie).limit(user_limit).offset(user_offset).all()
    ...
    def get_by_filters(self, user_offset=0, user_limit=None):
        return self.session.query(Movie).order_by(desc(Movie.year)).limit(user_limit).offset(user_offset).all()
